...
# .husky/pre-commit
#!/bin/sh
npx evnx scan --severity high     # Exits 1 on a high-confidence detection
...
✅ Weekly `evnx diff` runs to detect configuration drift
✅ New team members run `evnx init` to set up local env
```

<Callout type="warning">
Corrected September 2026: this policy listed `evnx audit` and `evnx onboarding`. Neither
command exists. Drift detection against a committed template is `evnx diff` — or
`evnx sync --check`, which is the one built for a CI gate and exits 1 when the two files are
out of step. Onboarding a new machine is `evnx init`.
</Callout>
