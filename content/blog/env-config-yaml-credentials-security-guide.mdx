...
[FIX] Add packages/api/.env to .gitignore, then: git rm --cached packages/api/.env
```

<Callout type="warning">
Corrected September 2026: this output previously suggested `evnx ignore --add`, which does
not exist. `evnx doctor` reports an unignored `.env`; fixing `.gitignore` is yours to do.
`evnx init` writes the entries for a new project.
</Callout>
...
Add `evnx scan --severity high` to your GitHub Actions or GitLab CI pipeline. Fail the build if secrets are detected in configuration files. (`--strict` is a `validate` flag, not a `scan` one — `scan` narrows with `--severity`.)
</Callout>
...
if ! command -v evnx &> /dev/null; then
  ...
  evnx scan --severity high --quiet || exit 1
fi
...
3. **Add automated scanning**
   ...
   # Add a pre-commit hook — two lines, written by you
   printf '#!/bin/sh\nevnx scan --severity high || exit 1\n' > .git/hooks/pre-commit
   chmod +x .git/hooks/pre-commit
   ```

   <Callout type="warning">
   Corrected September 2026: this block previously ran `evnx hook install --pre-commit` and
   `evnx scan --fix`. Neither exists. evnx does not install hooks for you, and `scan` has no
   `--fix` — a scanner that edited files holding live credentials is not a thing you want.
   `evnx validate --fix` is real, and repairs `.env.example` drift rather than secrets.
   </Callout>

4. **Update your CI/CD pipeline**
   - Add `evnx scan --severity high` as a required check
