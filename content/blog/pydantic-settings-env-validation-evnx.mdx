...
### Step 4: Add the pre-commit hook (do this once per repo)

```bash
printf '#!/bin/sh\nevnx scan --severity high || exit 1\n' > .git/hooks/pre-commit
chmod +x .git/hooks/pre-commit
```

`--severity high` blocks only on detections matching a known provider format. A hook that
fires on anything is a hook people disable. The `||` also catches exit code 2, which means
the scan could not run — you want that to fail too, rather than pass silently.

<Callout type="warning">
Corrected September 2026: this step previously ran `evnx install-hook` and quoted a ~180ms
average. There is no such command, and the number was invented. evnx deliberately does not
write to `.git/hooks` — that directory is yours, it is not version-controlled, and a tool
that silently installs itself there is hard to audit and harder to remove.
</Callout>
...
evnx migrate --to aws-secrets-manager --secret-name myapp/production
...
evnx backup --output .env.backup.enc
...
<Callout type="warning">
Corrected September 2026: the second hook previously passed `--fail-on-warning`, which does
not exist — `evnx doctor` takes an optional path and nothing else. It already exits non-zero
on a failed check, so the flag was not needed to make the hook work.
</Callout>
