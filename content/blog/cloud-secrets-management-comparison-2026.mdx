...
**This section described commands and a package that do not exist.** It has been rewritten
against the shipped CLI.

Retracted: `evnx link aws`, `evnx pull`, `evnx run`, `evnx push KEY=value`, the
`@evnx/sdk` npm package and its `EvnxClient`, and the claim that evnx ships its own
pre-commit hook. None of them were ever released. Also retracted: "automatic sync with AWS
Secrets Manager, Vault, or other backends" — `evnx migrate` is a **one-way push** to nine
destinations, Vault is not among them, and nothing syncs continuously.

`evnx run` is a real roadmap item, and this post is simply early. It will be restored here
when it ships, not before.
</Callout>

**evnx** is a local CLI with an end-to-end encrypted sync layer. The server stores
ciphertext only — encryption and decryption happen on your machine, so a breach of the
vault yields nothing readable.

**What it actually does:**
- A local CLI that works on `.env` files directly — scan, validate, diff, convert
- End-to-end encrypted sync of a `.env` file to a vault you control (`evnx cloud`)
- Team sharing with roles, and revocation that re-keys the vault rather than only removing a row
- One-way migration of an existing `.env` into nine destinations (`evnx migrate`), including AWS Secrets Manager
- On-demand encrypted backups of a local `.env` (`evnx backup`) — not transparent encryption at rest
...
# Sign in and bind this directory to a vault
evnx auth login
evnx cloud link myapp/development

# Pull the .env for this environment
evnx cloud pull

# Run your app the way you already do — evnx wrote a real .env file
npm run dev

# Push local changes back, as a new encrypted version
evnx cloud push

# Scan for secrets that should not be committed
evnx scan
```

`evnx cloud push` takes a **file**, not `KEY=value` pairs: edit `.env`, then push. Every
push is a new version, so `evnx cloud history` shows what changed and when.

To move an existing `.env` into a cloud provider's own store instead, that is `evnx migrate`
— a one-time, one-way export:

```bash
evnx migrate --to aws-secrets-manager --aws-profile myapp --dry-run
```

**Integration:**

There is no evnx SDK to import. That is the design rather than a gap — `evnx cloud pull`
writes an ordinary `.env`, so your application keeps reading `process.env` and needs to know
nothing about evnx:

```typescript
// lib/config.ts — no evnx import, by design
export const config = {
  database: {
...
The published npm packages are `@evnx/cli` and its five platform binaries. An earlier
version of this post imported `@evnx/sdk` and constructed an `EvnxClient`; neither has ever
existed.

<Callout type="tip" title="Pre-commit hook">
evnx does not ship a hook — you write a two-line one that calls `evnx scan`:
...
Save as `.git/hooks/pre-commit` and `chmod +x` it. Use `--severity high` so that only
high-confidence detections block a commit; a hook that fires on anything is a hook people
disable. Exit code 2 means the scan could not run at all, which you want to fail on too —
the `||` above covers both.

An earlier version of this post claimed evnx included a hook and quoted a 180ms cold start.
Neither was real.
</Callout>
