...
<Callout type="warning" title="Two commands this post originally promised do not exist">
Earlier versions of this article pointed at `evnx share` and `evnx rotate`. Neither shipped, and the reasons are worth stating rather than quietly deleting:

**Sharing** is blocked on post-quantum work, not on effort. Sharing a vault wraps its key with X25519 ECDH to the recipient's public key, and Shor's algorithm breaks X25519 — so a shared vault recorded today could be decrypted by a future adversary. A hybrid X25519 + ML-KEM-768 wrap lands in evnx-crypto 0.2.0 first. Your **own** copy of a vault key is wrapped symmetrically and is unaffected.

**Key rotation** is genuinely harder than it looks in a zero-knowledge system, for the reason the section above describes: rotating a master key means re-wrapping every vault key that depends on it, atomically. It is planned, it is not shipped, and pretending otherwise would be the wrong kind of documentation.

What exists today is in **[evnx cloud](/guides/commands/cloud)**.
</Callout>
