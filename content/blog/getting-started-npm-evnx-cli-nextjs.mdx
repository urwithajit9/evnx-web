...
```gitignore
# Environment variables — everything except the committed templates
.env*
!.env.example
!.env.sample
!.env.template
...
Updated September 2026. This block used to list `.env`, `.env.local` and `.env*.local`,
which leaves **`.env.production` and `.env.staging` tracked** — the files most likely to
hold real credentials. `.env*` with the three negations is what `evnx init` writes, and the
negations must come after the pattern they carve out of or git ignores them.

`.env.backup*` is no longer needed as its own line: `.env*` already covers it.
</Callout>
