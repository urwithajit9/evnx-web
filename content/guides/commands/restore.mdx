...
| `--password-file` | `path` | — | Read decryption password from a file (CI/CD). **v0.5.0+** — also accepted as `--key-file`, the spelling [`backup`](/guides/commands/backup) uses |
...
**1. `--password-file <path>`** — read from a file. UTF-8 content is trimmed of
surrounding whitespace; binary content is Base64-encoded first. This is the same
reader [`evnx backup --key-file`](/guides/commands/backup) uses, so one file
round-trips — **changed in v0.5.0**, when the two sides could still derive
different passwords from one file.
...
**Fix:** Rebuild with the feature flag (see [backup docs](/guides/commands/backup#backup-feature-not-enabled)).
