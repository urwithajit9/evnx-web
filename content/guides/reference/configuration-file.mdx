...
⚠️ **There is no `EVNX_<SECTION>_<KEY>` mechanism.** The variables evnx actually reads are
`EVNX_AUTO_FIX`, `EVNX_OUTPUT_JSON`, `EVNX_CONFIG_DIR`, `EVNX_PASSWORD`, `EVNX_SERVER` and
`EVNX_TOKEN` — nothing else. `EVNX_SCAN_MIN_SEVERITY` and friends are ignored, and the
commands they were shown with (`scan --exit-code`, `scan --output`) do not exist either.

Pass the flags directly:

```bash
evnx scan --severity high          # v0.5.0+
