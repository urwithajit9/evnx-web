...
- Creates a **blank** setup: an empty `.env.example`, a `.env`, and the
  `.gitignore` entries
- Skips preview and confirmation prompts
- Deterministic — the same commit scaffolds the same files every time
- Ideal for CI/CD, Docker builds, or scripting

<Callout type="warning">
**Corrected for v0.5.0.** This said `--yes` defaults to "Blueprint mode (first
available blueprint)", which was the non-determinism bug: the list came from a
hash map, so the chosen stack varied between runs. Use `--blueprint <name>` to
pick one, or `--with <components>` to name them.
</Callout>
...
|------|-----------|-----------------|
| `.env.example` | Only if missing | **Refused if it exists** — pass `--force` to replace it |
