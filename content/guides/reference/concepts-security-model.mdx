...
evnx backup --env-name test --output test.backup  # Interactive password
...
strace -f -e trace=network evnx backup .env.test --output net.backup 2>&1 \
  | grep -E "connect|sendto" || echo "✓ No network calls"
```

<Callout type="danger">
⚠️ **This check used to pass for the wrong reason.** It ran
`evnx backup --env .env.test` — and `backup` has no `--env` flag, the file is
positional. So the command exited 2 without doing anything, `grep` found no
network calls in the output of a program that never ran, and the checklist
printed `✓ No network calls`.

A verification step that passes when the thing it verifies did not execute is
worse than no step at all. If you are adapting this, confirm the command
actually runs before trusting the `✓`:

```bash
evnx backup .env.test --output net.backup; echo "exit=$?"   # expect 0
```
</Callout>
...
**Solution**: Use `--password-file` with secure secret injection:
...
- name: Restore backup
  ...
  run: |
    # `restore` reads the password from a FILE, never from an argument —
    # a command line is visible in `ps` and in CI logs.
    printf '%s' "$ENV_PASSWORD" > "$RUNNER_TEMP/pw"
    evnx restore backup.env --password-file "$RUNNER_TEMP/pw"
    shred -u "$RUNNER_TEMP/pw"
```

<Callout type="warning" title="Never hardcode passwords">
❌ Bad: a password as an argument — `--password` does not exist, and would be
visible in `ps` and in CI logs if it did.
✅ Good: `evnx restore backup --password-file "$RUNNER_TEMP/pw"`, with the file
written from a secret and removed afterwards.
...
<Callout type="tip" title="Do not take the client's side of this on trust">
The claims above that are observable from your own machine — that a file survives a round
trip unchanged, and that `evnx cloud run` keeps secrets out of `ps` — can be checked in
about fifteen minutes:
**[Verifying the Round Trip](/guides/reference/verifying-the-round-trip)**.

It is explicit about what it does *not* establish. A client-side test cannot prove what the
server does not hold; that comes from the architecture, not from an experiment.
</Callout>
