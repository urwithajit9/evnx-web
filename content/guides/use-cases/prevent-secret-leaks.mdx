...
Commands on this page previously used `--exit-code` (on `scan`, `doctor` and `diff`) and
`evnx doctor --fix`. **None of those exist.** `scan`, `doctor` and `diff` already exit
non-zero when they have something to report, and doctor's auto-fix is reached with
`EVNX_AUTO_FIX=1`. `--severity` needs **v0.5.0+**.
</Callout>
