...
```bash
# Dev
evnx template --input docker-compose.yml.template --output docker-compose.yml --env .env.dev --gitignore
...
# Production
evnx template --input docker-compose.yml.template --output docker-compose.yml.prod --env .env.production --gitignore
...
```bash
# Staging
evnx template --input config.toml.template --output config.toml --env .env.staging
...
2. **[Template Syntax Guide](/guides/commands/template)** — Deep dive into all three substitution styles and when to use each
3. **[CI/CD Integration](/guides/integrations/ci-cd-sync)** — Integrating `evnx template` into GitHub Actions, GitLab CI, and similar pipelines
4. **[Security Best Practices](/guides/use-cases/prevent-secret-leaks)** — Gitignore patterns, secrets manager integration, and auditing generated files
