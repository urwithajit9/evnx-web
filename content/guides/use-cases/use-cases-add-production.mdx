...
<Prerequisites>
  ...
  <Prerequisite href="/guides/getting-started/add-basic">Learn add basics</Prerequisite>
</Prerequisites>
...
- ✅ `.env` gets the variables with placeholder values, which `evnx validate` then names
...
# Team can now:
# - See which flags are optional vs required, from the comments
...
**`evnx validate --category` does not exist**, and neither does
`evnx validate --recursive` further down this page. Both were listed here as
shipped features. The section headers and `[Category]` comments are real —
`evnx add` writes them and they survive a round trip — but nothing filters or
validates by them. They are documentation for humans and for `git diff`.
</Callout>
...
# Validate all services — loop, since validate takes one file at a time:
for dir in ./services/*/; do
  echo "== $dir"
  evnx validate --env "$dir/.env" --example "$dir/.env.example" || exit 1
done
```
...
1. **[Sync Basics](/guides/getting-started/sync-basic)** — Keep `.env` and `.env.example` aligned
2. **[Validate Command](/guides/commands/validate)** — Catch issues before deployment
3. **[Team Collaboration](/guides/use-cases/team-collaboration)** — Share templates safely at scale
4. **[Schema Reference](/guides/commands/spec)** — Extend evnx with your own services and frameworks
