...
<Prerequisites>
  <Prerequisite href="/guides/getting-started/sync-basic">Complete sync basics</Prerequisite>
...
```bash
# Add to pre-commit hook or CI (evnx v0.5.0+):
# --check writes nothing and exits 1 when the template is stale, 2 on error.
if ! evnx sync --direction forward --check; then
...
```bash
# 1. Verify .env.example is up to date (exits 1 if stale, v0.5.0+)
...
- [Security Best Practices](/guides/reference/concepts-security-model) — Protect team secrets
