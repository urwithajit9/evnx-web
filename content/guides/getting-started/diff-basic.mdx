...
<Prerequisites>
  ...
  <Prerequisite href="/guides/getting-started/init-basic">Initialize a project</Prerequisite>
</Prerequisites>
...
Different values:
  ...
  1 missing from .env  ·  1 not in .env.example  ·  1 different value
```

<Callout type="info">
**Values are not shown by default.** `~ DATABASE_URL` tells you the two files
disagree, not what they say — pass `--show-values` if you want the values, and
only where it is safe to have them on screen.
</Callout>
...
1. **[Sync Basics](/guides/getting-started/sync-basic)** — Automatically reconcile differences instead of just detecting them
...
4. **[CI/CD Integration](/guides/integrations/ci-cd-sync)** — Enforce environment consistency in pipelines
