...
```text
  ...
  ✗  DEBUG is set to "yes" (string, not boolean)
     .env
     →  Use true or false for proper boolean handling
     ·  fixable with --fix
  ...
  validate:
    ...
    steps:
      ...
      - name: Validate production config
        run: evnx validate --env-name production --validate-formats --format json
```
...
**`--pattern` was removed in v0.5.0 — this page used it eleven times.** Use
`--env-name`, which takes the name rather than the filename:
`--env-name production` resolves `.env.production`.

The replacement is not cosmetic. `--pattern`'s whole implementation was a
`starts_with(".env.")` test — anything else printed *"Pattern 'x' not
recognized, using default"* and validated `.env` instead, **exiting `0`**. So a
CI job with a typo'd pattern silently checked the wrong file and passed.
`--env-name` errors on a name that resolves to nothing, and lists the
environments that do exist.
</Callout>

```bash
# Development: lenient, skip Docker warnings
evnx validate --env-name local --ignore localhost_in_docker

# Staging: moderate checks
evnx validate --env-name staging --validate-formats
...
validate-prod:
	evnx validate --env-name production --strict --validate-formats
...
# Multi-Environment
# ─────────────────────────────────────
evnx validate --env-name local         # Dev
evnx validate --env-name staging       # Staging
evnx validate --env-name production --validate-formats  # Prod
...
- **[Setting up .env.example](/guides/commands/sync)** — best practices for environment templates
- **[Secrets management](/guides/use-cases/prevent-secret-leaks)** — when to use evnx vs. cloud secret managers
- **[CI/CD integration](/guides/integrations/ci-cd-sync)** — full pipeline examples with evnx
