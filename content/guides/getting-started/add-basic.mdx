...
Both files carry the same variables. `.env` is yours to fill in; `.env.example` is
the template you commit.

**Step 4: Replace placeholders with real values**

```bash
nano .env
# Replace each placeholder value with your actual credentials
# Keep .env.example unchanged (it's your team template)

evnx validate        # names exactly which values still need replacing
```

<Callout type="info">
  **Changed in v0.5.0.** `.env` used to receive these as `# TODO:` comment lines. The
  parser cannot see a commented assignment, so `evnx add` reported adding six variables
  and `evnx validate`, run immediately after, reported all six *missing*. Both commands
  now agree.
</Callout>
...
1. **[Sync Basics](/guides/getting-started/sync-basic)** — Keep `.env` and `.env.example` in sync
2. **[Validate Command](/guides/commands/validate)** — Check your environment for issues
3. **[Team Collaboration](/guides/use-cases/team-collaboration)** — Share templates safely with your team
4. **[Schema Reference](/guides/commands/spec)** — Learn to extend services and frameworks
